...
public class GuestRepository {
    ...
    public List<GuestShortResponseDto> getAllGuests(GuestFilter filter) {
        //language=PostgreSQL
        String sql = "SELECT guest_id AS id, first_name AS firstName, last_name AS lastName, passport_number AS passportNumber, is_inhabited AS isInhabited FROM guests " +
                "WHERE 1=1 ";
    ...
    public Optional<GuestEntity> getGuestById(long guestId) {
        ...
        String sql = "SELECT  guest_id AS id, passport_number AS passportNumber, last_name AS lastName, first_name AS firstName, " +
                "middle_name AS middleName, city AS city, check_in_date AS checkInDate, is_inhabited as isInhabited, room_id as roomId " +
                "FROM guests " +
